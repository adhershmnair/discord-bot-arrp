  ...
  async execute(interaction) {
    try {
        ...
        if (leaveData.length === 0) {
          error(`<@${interaction.member.id}> tried to get leave data for <@${targetUser.id}> but no data found.`, leaveLogChannel);
        ...
        if (leaveType === 'summary') {
          ...
          log(`<@${interaction.member.id}> got leave summary for <@${targetUser.id}>`, leaveLogChannel);
        ...
        const fields = leaveData.map(leave => {
          ...
          return [
            ...
            { name: 'Active', value: leave.is_active ? "**Yes**" : "No", inline: true },
            { name: '\u200B', value: '\u200B' }, // Empty row for spacing
          ];
        }).flat();
        interaction.reply({ content: leaveTitle, ephemeral: true, embeds: [{ fields }] });
        log(`<@${interaction.member.id}> got leave data for <@${targetUser.id}>`, leaveLogChannel);
